...
@section("content")
    ...
    <form action="/admin/content{{(is_null($model)) ? "" : "/" . $model->id}}" class="form-horizontal"
        ...
        @csrf
            ...
            <input
                    name="language_content_id" type="hidden"
                    value="{{(is_null($model)) ? request()->segment(4) : $model->language_content_id }}"/>
            <input
                    name="language_id" type="hidden"
                    value="{{(is_null($model)) ? request("language") : $model->language_id }}"/>
        <div class="modal-dialog modal-lg" role="document" style="width: 1000px">
            ...
            <div class="card">
                <div class="modal-header">
                    <h4 class="modal-title bold m-5">
                        @if(is_null($model))
                        {{$content->title}} {{ config("app.locales")[request()->input("language")]}} Dili Ekle
                            @else
                       İçeriği Güncelle
                            @endif
                    </h4>
